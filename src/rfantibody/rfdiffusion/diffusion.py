...
class IGSO3():
    ...
    def __init__(self, *, T, min_sigma, max_sigma, min_b, max_b,
        ...
        _to_float = lambda x: x.float() if isinstance(x, torch.Tensor) else torch.from_numpy(x).float()
        self._discrete_omega = _to_float(self.igso3_vals['discrete_omega'])
        self._discrete_sigma = _to_float(self.igso3_vals['discrete_sigma'])
        self._score_norm_table = _to_float(self.igso3_vals['score_norm'])
