...
class Denoise():
    ...
    def align_to_xt_motif(self, px0, xT, diffusion_mask, eps=1e-6):
        ...
        _det_input = (Vh.T @ U.T)
        if _det_input.device.type == 'mps':
            _det_val = torch.linalg.det(_det_input.cpu()).to(_det_input.device)
        else:
            _det_val = torch.linalg.det(_det_input)
        d[-1, -1] = torch.sign(_det_val)
